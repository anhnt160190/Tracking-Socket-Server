...
async function MapDetail() {
  ...
  var TableView = function () {
    ...
    this.onAddVehicleTracking = function () {
      ...
      var area = $(
        "<div class='area-tracking box' style='position:absolute; width: 190px; bottom: 2px; left: 2px; z-index:999; padding: 1px'>"
      );
    ...
    this.viewTracking = function (tracking, focus) {
      if (focus) {
        ...
        parent.scrollTop(
          parent.scrollTop() +
            tracking.row.position().top -
            parent.height() / 2 +
            tracking.row.height() / 2
        );

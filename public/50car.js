$(function () {
  ...
  Core.Timer = function (option) {
    ...
    this.start = function () {
      ...
      var interval =
        typeof option.interval == 'function'
          ? option.interval
          : function () {
  ...
  var Online = function () {
    ...
    this.active = function (vehicle) {
      if (currentVehicleActive == null) activeHelper(vehicle);
        ...
        withView(function (v) {
          v.deactive(currentVehicleActive);
        });
        if (currentVehicleActive.state.VehiclePlate != vehicle.state.VehiclePlate)
          activeHelper(vehicle);
        else {
  ...
  var TableView = function () {
    ...
    this.onAdd = function () {
      ...
      var area = $(
        "<div class='area-online box' style='position:absolute; width: 190px; bottom: 2px; left: 2px; z-index:999; padding: 1px'>"
      );
    ...
    this.active = function (vehicle) {
      vehicle.row.addClass('bg-success text-white');
    };
    this.deactive = function (vehicle) {
      vehicle.row.removeClass('bg-success text-white');
    };
  };
  var MapView = function () {
    ...
    var vehicleIcons = [];
    for (var i = 0; i <= 7; i++) {
      vehicleIcons.push('icon/{0}.png'.format(i));
    }
    ...
    this.show = function (vehicle) {
      if (vehicle.marker == null) {
        ...
        var iconElement = $(
          "<div><div class='badge badge-secondary' style='font-size:10px'>{0}</div><img src='' /></div>".format(
        ...
        vehicle.marker.on('click', function () {
          $this.online.active(vehicle);
        });
      } else {
        if (vehicle.oldState == null) vehicle.direction = Core.random(8);
        else
          vehicle.direction = getDir(
            vehicle.direction,
            vehicle.oldState.v,
            vehicle.oldState.k,
            vehicle.state.v,
        ...
        if (vehicle.active) this.active(vehicle);
      }
    };
    this.active = function (vehicle) {
  ...
  var ViturlSocket = function (online, data) {
    ...
    this.start = function () {
      onVehicle(function (vehicle) {
        vehicle.currentTrackingIndex = 0;
      });
